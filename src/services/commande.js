...
export async function getCommandeByCustomer(id_customer) {

    const response = await fetch(
        `/api/orders?filter[id_customer]=[${id_customer}]&display=full`,
        { headers }
    );

    if (!response.ok) {
        throw new Error("Impossible de recuperer les commandes");
    }
    ...
    const list = Array.isArray(commandeList) ? commandeList : [commandeList];

    const fullCommandes = await Promise.all(
        list.map(async (c) => {
            // 🆕 chercher l'id dans tous les endroits possibles
            const id = c["@_id"] ?? c.id ?? getText(c.id);

            if (!id) return null; // 🆕 sécurité si vraiment pas d'id
...
export async function updateOrderState(orderId, stateId) {
  ...
  `.trim();
    ...
    const res = await fetch("/api/order_histories", {
        method: "POST",
        headers: {
            ...headers,
...
export async function getOrderStates(languageId = "1") {
    const res = await fetch("/api/order_states", { headers });
    ...
    const details = await Promise.all(
        ids.map(async (id) => {
            const detailRes = await fetch(`/api/order_states/${id}`, { headers });
...
export async function get_order_state(current_state) {
    try {

        const response = await fetch(
            `/api/order_states/${current_state}/1`,
            {
                headers,
            }
        );
        ...
        if (!response.ok) {
            throw new Error("Impossible de recuperer l'etat de commande");
        }
        ...
        const state =
            data.prestashop.order_state.name.language;
